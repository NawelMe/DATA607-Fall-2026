...
I started with the movie ratings spreadsheet and treated blank cells as movies a critic had not rated yet. I changed the data into a format with one row for each critic and movie, which made it easier to calculate averages. First, I found the overall average rating. Then I looked at how each movie and each critic differed from that average. I added those three pieces together to predict the missing ratings.

I checked my calculation using the example for Param and Pitch Perfect 2, which came out to about 2.28. After that, I predicted the other missing ratings and chose the highest one as a recommendation for each critic. Finally, I used the same steps on the small Week 2 sample to see how the method worked with another set of ratings.
...
[View the published video on YouTube](https://www.youtube.com/watch?v=ud3t9keVRAU)
